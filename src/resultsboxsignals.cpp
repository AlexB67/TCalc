                                                     ...
                                                     const Glib::RefPtr<Gtk::Tooltip>& tooltip)
{
    ...
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[12])
        tooltip->set_markup(_("The focal length corresponding to the low magnification limit."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[13])
        tooltip->set_markup(_("The focal length corresponding to the high magnification limit."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[14]) 
        tooltip->set_markup(_("Power per inch of aperture at the current magnification."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[15]) 
        tooltip->set_markup(_("The amount of light gathered by the telescope."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[16]) 
    ...
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[17]) 
    ...
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[22]) 
    ...
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[23]) 
        tooltip->set_markup(_("The airy disk size, a smaller disk means more resolving power."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[24]) 
    ...
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[25])
        tooltip->set_markup(_("The smallest possible object that can be seen on the moon (excluding seeing conditions)."));
    else if(row->get_value(m_resultCols.m_results_property) == m_proplistnames[26]) 
                                                     ...
                                                     const Glib::RefPtr<Gtk::Tooltip>& tooltip)
{
    ...
    if (row->get_value(m_scopeCols.m_results_property) == m_scopelistnames[0])
    {
       ...
       {
            (row->get_value(m_scopeCols.m_results_set1).length()) ?
            tooltip->set_markup(row->get_value(m_scopeCols.m_results_set1)) : tooltip->set_markup(tip + _("."));
            m_scopeview.set_tooltip_cell(tooltip, &path, nullptr, nullptr);
            return true;
       }
       else if (column->get_title() == _("Set 2"))
       {
            (row->get_value(m_scopeCols.m_results_set2).length()) ?
            tooltip->set_markup(row->get_value(m_scopeCols.m_results_set2)) : tooltip->set_markup(tip + _("."));
            m_scopeview.set_tooltip_cell(tooltip, &path, nullptr, nullptr);
            return true;
       }
    }
                                                    ...
                                                    const Glib::RefPtr<Gtk::Tooltip>& tooltip)
{
    ...
    if (row.get_value(m_epCols.m_results_property) == m_eplistnames[0])
    {
       ...
       {
            (row.get_value(m_epCols.m_results_set1).length()) ?
       ...
       {
            row.get_value(m_epCols.m_results_set2).length() ?
