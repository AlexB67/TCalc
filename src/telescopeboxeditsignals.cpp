...
void ScopeBox::EditTelescopes::set_signal_handlers()
{
    ...
    m_smodel->signal_changed().connect(sigc::mem_fun(*this, &EditTelescopes::on_smodel_edit_changed));
    m_button_new.signal_clicked().connect(sigc::mem_fun(*this, &EditTelescopes::on_new_button_clicked));
    m_button_edit.signal_clicked().connect(sigc::mem_fun(*this, &EditTelescopes::on_edit_button_clicked));
    m_button_cancel.signal_clicked().connect(sigc::mem_fun(*this, &EditTelescopes::on_cancel_button_clicked));
    m_button_save.signal_clicked().connect(sigc::mem_fun(*this, &EditTelescopes::on_save_button_clicked));
    m_button_del.signal_clicked().connect(sigc::mem_fun(*this, &EditTelescopes::on_del_button_clicked));
}
...
void ScopeBox::EditTelescopes::on_smodel_edit_changed()
{
    ...
    if (iter)
    {
        ...
        if (row)
        {
            ...
            (false == tmp.empty()) 
            ? m_smirrorcoating.set_active_text(tmp) : m_smirrorcoating.set_active_text(_("unknown"));

            tmp = static_cast<Glib::ustring>(row[m_scombomodel.m_scopecols.m_smirrormaterial]);
            (false == tmp.empty()) 
            ? m_smirrormaterial.set_active_text(tmp) : m_smirrormaterial.set_active_text(_("unknown"));

            tmp = static_cast<Glib::ustring>(row[m_scombomodel.m_scopecols.m_slenscoating]);
            (false == tmp.empty()) 
            ? m_slenscoating.set_active_text(tmp) : m_slenscoating.set_active_text(_("unknown"));

            tmp = static_cast<Glib::ustring>(row[m_scombomodel.m_scopecols.m_slensmaterial]);
            (false == tmp.empty()) 
            ? m_slensmaterial.set_active_text(tmp) : m_slensmaterial.set_active_text(_("unknown"));
            ...
            (false == tmp.empty()) 
            ? m_smount_type.set_active_text(tmp) : m_smount_type.set_active_text(_("unknown"));

            tmp = static_cast<Glib::ustring>(row[m_scombomodel.m_scopecols.m_sfocuser_type]);
            (false == tmp.empty()) 
            ? m_sfocuser_type.set_active_text(tmp) : m_sfocuser_type.set_active_text(_("unknown"));

            tmp = static_cast<Glib::ustring>(row[m_scombomodel.m_scopecols.m_sfinder_type]);
            (false == tmp.empty()) 
            ? m_sfinder_type.set_active_text(tmp) : m_sfinder_type.set_active_text(_("unknown"));
        }
    }
...
void ScopeBox::EditTelescopes::on_new_button_clicked()
{
    enable_widgets(true);
    set_default_values();
...
void ScopeBox::EditTelescopes::on_edit_button_clicked()
{
    enable_widgets(true);
    m_button_save.set_sensitive(true);
    m_button_cancel.set_sensitive(true);
...
void ScopeBox::EditTelescopes::on_cancel_button_clicked()
{
...
void ScopeBox::EditTelescopes::on_save_button_clicked()
{
    m_smodellabel.set_label(_("Select telescope"));
    ...
    ? std::get<7>(scopedata) = "" : std::get<7>(scopedata) = m_smirrorcoating.get_active_text();
    
    (_("unknown") == m_smirrormaterial.get_active_text()) 
    ? std::get<8>(scopedata) = "" : std::get<8>(scopedata) = m_smirrormaterial.get_active_text();

    (_("unknown") == m_slenscoating.get_active_text()) 
    ? std::get<9>(scopedata) = "" : std::get<9>(scopedata) = m_slenscoating.get_active_text();

    (_("unknown") == m_slensmaterial.get_active_text()) 
    ? std::get<10>(scopedata) = "" : std::get<10>(scopedata) = m_slensmaterial.get_active_text();

    (m_sstrehl.get_value() < Astrocalc::astrocalc::tSMALL) 
    ? std::get<11>(scopedata) = 0 : std::get<11>(scopedata) = m_sstrehl.get_value();

    (m_sweight.get_value() < Astrocalc::astrocalc::tSMALL) 
    ? std::get<12>(scopedata) = 0 : std::get<12>(scopedata) = m_sweight.get_value();

    (m_stube_weight.get_value() < Astrocalc::astrocalc::tSMALL) 
    ? std::get<13>(scopedata) = 0 : std::get<13>(scopedata) = m_stube_weight.get_value();

    (_("unknown") == m_smount_type.get_active_text()) 
    ? std::get<14>(scopedata) = "" : std::get<14>(scopedata) = m_smount_type.get_active_text();

    (_("unknown") == m_sfocuser_type.get_active_text()) 
    ? std::get<15>(scopedata) = "" : std::get<15>(scopedata) = m_sfocuser_type.get_active_text();

    (_("unknown") == m_sfinder_type.get_active_text()) 
    ? std::get<16>(scopedata) = "" : std::get<16>(scopedata) = m_sfinder_type.get_active_text();
...
void ScopeBox::EditTelescopes::on_del_button_clicked()
{
